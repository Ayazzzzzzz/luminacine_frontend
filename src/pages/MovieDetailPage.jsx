...
export default function MovieDetailPage() {
  ...
  const isRescheduling = new URLSearchParams(location.search).get('reschedule') === 'true'
  ...
  useEffect(() => {
    // Pindahkan pengecekan accessToken ke sini
    if (!accessToken) {
      navigate('/');
      return;
    }

    // Pastikan id ada sebelum fetch data
    if (!id) {
      console.error("ID film tidak ditemukan di URL");
    ...
    const fetchSchedules = async () => {
      try {
        ...
        setError('Gagal memuat jadwal tayang')
      } finally {
        setLoading(false)
      }
    }
  ...
  if (!movie) {
    return <div className="min-h-screen bg-neutral-900 text-white flex justify-center items-center">Film tidak ditemukan</div>
  }
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            ...
            {filteredSchedules.length > 0 ? (
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    if (isRescheduling && bookingId) {
                      // Kirim ke halaman Choose Seat dulu, bawa semua data lewat query param
                      navigate(`/movies/${id}/schedule/${selectedScheduleId}/choose-seat?reschedule=true&bookingId=${bookingId}&oldPrice=${selectedSchedule.price}`);
                    } else {
                      // Booking biasa
              ...
              <p className="text-neutral-400">Tidak ada jadwal tayang untuk tanggal ini.</p>
            )}
          </div>
        </div>
      </main>
