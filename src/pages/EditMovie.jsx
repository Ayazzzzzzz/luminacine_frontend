...
export default function EditMovie() {
  ...
  useEffect(() => {
    axios
      .get(`https://luminacine-be-901699795850.us-central1.run.app/movies/${id}`)
      .then((res) => {
        ...
        setPosterName("Current Poster (unchanged)");
      })
      .catch((err) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Upload new poster jika diganti
      if (formData.posterFile) {
        ...
        posterUrl = cloudinaryRes.data.secure_url;
      }
      ...
      await axios.put(
        `https://luminacine-be-901699795850.us-central1.run.app/movies/${id}`,
        updatedMovie
      );

      alert("Movie berhasil diupdate!");
      navigate("/admindashboard");// arahkan balik ke halaman utama
    } catch (error) {
      ...
      <div className="flex flex-col md:flex-row items-start justify-between gap-16 w-full max-w-7xl">
        <div className="w-full md:w-2/3 ml-6">
          <div className="flex items-center gap-2 mt-0 mb-2">
            ...
            <h1 className="text-yellow-400 text-[48px] font-serif font-bold">EDIT MOVIE</h1>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-black">
            <div>
              ...
              <input
                ...
                className="w-full bg-[#f1f1f1] rounded-md px-3 py-3 text-[16px]"
              />
            ...
            <div>
              ...
              <textarea
                ...
                onChange={handleChange}
                rows={4}
                className="w-full h-16 bg-[#f1f1f1] rounded-md px-4 py-3 text-[16px]"
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                className="w-full bg-[#f1f1f1] rounded-md px-4 py-3 text-[16px]"
              />
            ...
            <div>
              ...
              />
              {posterName && <p className="mt-2 text-white text-sm italic">File: {posterName}</p>}
            </div>
